...
class Node
{
	private:
		...
		Node* parent;
	public:
		Node(string key, Contact *data): key(key),left(NULL),right(NULL),parent(NULL) {contactVector.push_back(data);}
...
class ContactBST
{
	...
	public:
		...
		int exportCSV(Node* ptr,ofstream& file); //Export all the contacts from the BST to a CSV file in an ascending order
	private:
